...
The EPAC script `new-az-remediation-tasks.sh` is written to leverage the EPAC system to automate enforcement of as many as all of the policyAssignments that exist in EPAC in a single line of code. While this is extremely powerful, it is intended to be automation that leverages the native [az policy remediation create](https://learn.microsoft.com/en-us/cli/azure/policy/remediation#az-policy-remediation-create) command at Enterprise Scale. If this scale is not desired, it is recommended to simply use `az policy remediation create` from the [Azure CLI](https://learn.microsoft.com/en-us/cli/azure/) to accomplish this goal until a steady state can be reached for a given assignment, which should at that time be transitioned to the Steady State procedure below.
